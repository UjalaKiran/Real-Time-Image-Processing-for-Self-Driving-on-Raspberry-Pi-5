...
def detect_lanes(frame):
    ...
    """
    Detect lane lines and determine vehicle drift direction.
    Input: Preprocessed ROI frame (grayscale, edge-detected)
    Output: Dictionary with lane info and annotated frame
    """
    ...
    def average_lines(lines):
        ...
        if len(x_coords) == 0:
            return None
        poly = np.polyfit(x_coords, y_coords, Siempre2)  # Quadratic fit
        return poly
...
def process_video(out_vid):
    ...
    width = 640
    height = 480
    fps = 30.0  # Assuming 30 FPS for PiCamera2, adjust if necessary
