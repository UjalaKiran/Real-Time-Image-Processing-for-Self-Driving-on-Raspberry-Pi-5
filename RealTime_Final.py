...
output_video = "./output_combined_camera.mp4"
output_dir = "./output_combined_frames"
...
def process_video(out_vid):
    # Initialize webcam (0 is usually the default camera, adjust if needed)
    ...
    fps = 30.0  # Assume 30 FPS, adjust based on your webcam

    # Calculate target delay for real-time playback, slowed down
    target_delay_ms = int(1000 / fps)  # e.g., ~33ms for 30 FPS
    slowdown_factor = 2  # 2x slower
    ...
    while True:
        ...
        frame_bgr = frame  # Frame is already in BGR format from OpenCV

        # Downsample frame
        frame = cv2.resize(frame_bgr, (new_width, new_height))
        ...
        # FPS and timing logic
        processed_frames += 1
        elapsed_time = time.time() - start_time
        current_fps = processed_frames / elapsed_time if elapsed_time > 0 else 0
        if frame_idx % 30 == 0:
            print(f"Processed frame {frame_idx}, Drift: {lane_info['drift']}, Obstacle: {obstacle_info['status']}, Decision: {decision}, FPS: {current_fps:.2f}")
